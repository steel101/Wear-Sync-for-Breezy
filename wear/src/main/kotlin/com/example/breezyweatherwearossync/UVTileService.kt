...
class UVTileService : TileService() {
    ...
    private fun getUvColor(uv: String): ColorBuilders.ColorProp {
        ...
        return when {
            value <= 2 -> ColorBuilders.argb(0xFF00E676.toInt()) // Low - Green
            value <= 5 -> ColorBuilders.argb(0xFFFFFF00.toInt()) // Moderate - Yellow
            value <= 7 -> ColorBuilders.argb(0xFFFF9800.toInt()) // High - Orange
            value <= 10 -> ColorBuilders.argb(0xFFFF5252.toInt()) // Very High - Red
            else -> ColorBuilders.argb(0xFF9C27B0.toInt()) // Extreme - Violet/Purple
        }
    }
